...
/// KubeError를 일관된 형식으로 로깅하는 헬퍼 함수
        ...
        match (nodes_res, pods_res) {
            (Ok(nodes), Ok(pods)) => {
                ...
                for node in nodes.items {
                    ...
                    // GPU 정보 추출
                    let gpu_model = node_labels.get("nvidia.com/gpu.product")
                        .cloned()
...
async fn main() -> std::io::Result<()> {
    ...
    // 2. 환경 변수로부터 외부 토큰 인증 클러스터 정보 로드 (주석 처리됨)
    /*
        ...
        match url.parse() {
            Ok(uri) => {
                ...
                if let Ok(ca_path) = external_ca_cert_path {
                    ...
                    warn!("K8S_EXTERNAL_CA_CERT_PATH 환경 변수가 설정되지 않았습니다.");
                }
    ...
    */
    ...
    let kube_contexts = web::Data::new(Arc::new(KubeContexts {
        contexts: successfully_connected_contexts
    }));
