...
// --- 인증 토큰 갱신 함수 (예시) ---
async fn refresh_token_if_needed(config: &mut Config) -> Result<(), Box<dyn std::error::Error>> {
...
async fn main() -> std::io::Result<()> {
    ...
    for context in &kubeconfig.contexts {
        ...
        client_creation_futures.push(tokio::spawn(async move {
            ...
            Ok((context_name, client)) as Result<(String, Client), Box<dyn std::error::Error + Send + Sync>>
        }));
    }
